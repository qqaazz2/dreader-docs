...
自 **DReaderServer v1.1.1** 起，  
发布的 ZIP 包中已**内置 DReader 的 Web 端程序**（⚠️ 不一定是最新版本）。

#### 使用内置 Web 端

1. 部署并启动 **DReaderServer**
2. 在浏览器中访问：
   - **IP / 域名**：与 DReaderServer 相同  
   - **端口**：Nginx 配置的端口（默认：`9070`）
3. 即可直接打开并使用 DReader Web 端，无需额外部署

#### 使用最新版本的 Web 端（可选）

如果你希望使用**最新版本**的 DReader Web 端：

1. 前往 **DReader 项目页面**下载最新的 Web 端程序
2. 解压 Web 端的 ZIP 包
3. 将解压后的文件放置到 **DReaderServer 根目录下的 `www` 文件夹**
4. 重启服务后，即可通过浏览器访问最新版本的 Web 页面
 
> Web 端与服务端为前后端分离架构，只要接口版本兼容，即可自由替换 Web 端版本。
